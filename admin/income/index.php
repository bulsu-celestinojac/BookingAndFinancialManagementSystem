...
// Get a list of all years with income data for the filter dropdown
$yearsResult = $conn->query("SELECT DISTINCT YEAR(date) AS year FROM income ORDER BY year DESC");
$availableYears = [];
...
<body class="bg-main-bg font-sans antialiased text-dark-text flex">
    ...
    <main class="flex-1 p-6 lg:ml-64 overflow-y-auto">
        ...
        <?php
        $totalResult = $conn->query("SELECT SUM(amount) as total FROM income WHERE 1=1 " . (!empty($selectedMonth) ? "AND MONTH(date) = $selectedMonth" : "") . (!empty($selectedYear) ? " AND YEAR(date) = $selectedYear" : ""));
